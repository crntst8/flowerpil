...
/**
 * Cloudflare Pages Function for /embed/* routes
 * Proxies embed HTML requests to the backend origin
 */
  ...
  for (const { targetHost, scheme, resolveOverride, sourceRequest } of attempts) {
    try {
      ...
      if (response.ok || !EDGE_ERROR_CODES.has(response.status)) {
        // Clone response to modify headers for embed security
        const newHeaders = new Headers(response.headers);
        newHeaders.set('Content-Security-Policy', 'frame-ancestors *');
        newHeaders.set('Permissions-Policy', 'clipboard-write=*, autoplay=*');
